...
`README.md`). Ordered by the agent's own priority; addressed items are
struck through with a pointer to where the fix landed.
...
1. ~~**Full-page reloads discard exploration state.** `/pokemon/favorite`,
   ...
   first.~~ **Addressed** in PR #89, "Replace Dashboard/Transactions
   full-page reloads with htmx partial swaps".
2. ~~**In-app Wiki is stale.** `templates/wiki.html`'s `#import` section
   ...
   cheap, no schema change.~~ **Addressed** — see the two "Wiki" entries
   below (content fix, then a separate visual-lift pass).
