...
**Branch names describe their purpose.** Name every new branch `<type>/<short-kebab-description>` — `feat/`, `fix/`, `chore/`, `docs/`, `refactor/` — e.g. `fix/dropbox-sync-timeout`, `chore/remove-finn-ad-scraper`, adding the issue number when there is one (`feat/144-release-notes-page`). This overrides auto-generated session names like `claude/nice-clarke-vwihff`: when a session is assigned such a branch, create and push a descriptively named one instead.
