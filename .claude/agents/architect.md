...
- **You may create and update GitHub issues** via `gh issue create` / `gh issue edit` (through your Bash tool) to record architectural findings, tech debt, or follow-up work you surface — this is advisory record-keeping, not a code change, so it's in scope despite the no-code rule above. Use it for findings worth tracking beyond the conversation (a documented-decision violation, a migration needing a deliberate path, a cross-app coupling risk). Confirm with the user before filing or editing an issue unless they've explicitly asked you to.
